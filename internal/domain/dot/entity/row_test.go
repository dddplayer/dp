...
func TestLineRow(t *testing.T) {
	...
	for i, f := range fs {
		expectedName := f.Name()
		expectedPort := valueobject.GenerateShortURL(f.ID())
...
func TestBuildNodeRow(t *testing.T) {
	...
	obj := &DummyDotElement{NameVal: "object", PortVal: "port", ColorVal: "color", AttributesVal: []arch.Nodes{[]arch.Node{attr1}}}

	// Define some test input values
	ms := []arch.Node{attr2}
	as := []arch.Node{attr1, attr2}
	isFirstRow := true
	right := 2
	row := 3
...
func TestLeftRightRow(t *testing.T) {
	...
	expectedNode := &Node{Table: &Table{Rows: expectedRows}}
	...
	// Define a test object
	obj = &DummyDotElement{NameVal: "object", PortVal: "port", ColorVal: "color",
		AttributesVal: []arch.Nodes{[]arch.Node{attr1, attr2}}}
...
func nodesEqual(a, b *Node) bool {
	...
	for i := range a.Table.Rows {
		if !rowsEqual(a.Table.Rows[i], b.Table.Rows[i]) {
			return false
		}
	}
	return true
...
func (e *DummyDotElement) ID() string {
	return e.PortVal
}
...
func (e *DummyDotElement) Children() []arch.Nodes {
...
func (a DummyDotAttribute) ID() string {
	return a.port
}
